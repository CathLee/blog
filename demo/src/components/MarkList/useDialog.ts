 ...
 * @LastEditTime: 2022-11-27 21:34:53
 * @FilePath: /demo/src/components/MarkList/useDialog.ts
 */
...
const useDialog = () => {
    ...
    const autoGetInfo = (type: string) => {
        ...
        request({
            url: 'https://www.duwanyu.com/spider/getIconAndTitle',
            method: 'post',
            data: { targetUrl: dialogFormState.targetUrl },
        })
            .then((res: { data: { title: string; icon: string } }) => {
                if (type === 'title') {
